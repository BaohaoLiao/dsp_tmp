...
def setup(args):
    ...
    for data_name in data_list:
        results.append(main(prm, draft_tokenizer, target_tokenizer, prm_tokenizer, data_name, args))
...
def get_responses(args, prm, draft_tokenizer, target_tokenizer, prm_tokenizer, prompts):
    ...
    while current_prompts:
        ...
        input_ids = [input_id[-4096:] if len(input_id)>=4096 else input_id for input_id in input_ids]
        ...
        good_prompts = []
        bad_prompts = []
        for (orig_idx, prompt, prev_responses), draft_output, step_reward in zip(current_prompts, draft_outputs, step_rewards):
        ...
        if bad_prompts:
            ...
            target_llm = LLM(
                model=args.target_name_or_path,
                tensor_parallel_size=1,
                pipeline_parallel_size=1,
                trust_remote_code=True,
                gpu_memory_utilization=0.9,
            ...
            for (orig_idx, prompt, prev_responses), target_output in zip(bad_prompts, target_outputs):
                good_prompts.append((orig_idx, prompt, prev_responses, target_output, False))  # False means use client2

            # delete previous llm engine
            destroy_model_parallel()
            del target_llm.llm_engine.model_executor.driver_worker
            del target_llm 
            gc.collect()
            torch.cuda.empty_cache()
        ...
        next_prompts = []
        for orig_idx, prompt, prev_responses, out, used_client1 in sorted(good_prompts, key=lambda x: x[0]):
            response_text = out.outputs[0].text + args.step_word
            client_id = 1 if used_client1 else 2
            #tokenizer = tokenizer1 if client_id == 1 else tokenizer2
            num_tokens = len(out.prompt_token_ids) #len(tokenizer.encode(response_text))
...
def main(prm, draft_tokenizer, target_tokenizer, prm_tokenizer, data_name, args):
