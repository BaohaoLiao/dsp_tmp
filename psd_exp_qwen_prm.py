...
def prm_scores(prm, prm_tokenizer, current_prompts, responses, batch_size=16):
    def make_step_rewards(logits, token_masks):
        ...
        for i in range(batch_size):
            ...
            valid_probs = sample[token_masks[i]]
    ...
    all_rewards = []
    for i in range(0, len(full_responses), batch_size):
        batch_messages = full_responses[i:i + batch_size]
        encoded_batch_messages = prm_tokenizer(batch_messages, return_tensors="pt", padding=True, truncation=True)
        padded_input_ids = encoded_batch_messages["input_ids"].to(prm.device)
        attention_mask = encoded_batch_messages["attention_mask"].to(prm.device)

        # Get model outputs
        with torch.no_grad():
            outputs = prm(input_ids=padded_input_ids, attention_mask=attention_mask)

        # Create token masks for the mini-batch
        token_masks = (padded_input_ids == step_sep_id)

        # Get step rewards for the mini-batch
        step_rewards = make_step_rewards(outputs[0], token_masks)
        all_rewards.extend(step_rewards)
        
        # Optional: Clear CUDA cache after each mini-batch
...
def get_responses(args, client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, prompts, problems):
    ...
    while current_prompts:
        ...
        step_rewards = prm_scores(prm, tokenizer_prm, current_prompts, responses1)
        ...
        if bad_prompts:
            ...
            step_rewards = prm_scores(prm, tokenizer_prm, bad_prompts, responses2)
...
def main(client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        outputs, token_counts, turn_info, draft_rewards, target_rewards, draft_responses, target_responses = get_responses(
            args,
            client1, 
            client2,
            prm,
