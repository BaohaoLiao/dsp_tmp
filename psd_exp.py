...
def get_responses(args, client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, prompts, problems):
    ...
    while current_prompts:
        ...
        if bad_prompts:
            ...
            full_bad_responses = [''.join(r[0] for r in prev_resp) + new_resp.text
                    for (_, _, prev_resp), new_resp in zip(bad_prompts, responses2)]
            processed_bad_data = [
                prepare_input(problems[orig_idx], full_resp, tokenizer=tokenizer_prm, step_token=args.step_word) 
                for (orig_idx, _, _), full_resp in zip(bad_prompts, full_bad_responses)
            ]
            input_ids, steps, reward_flags = zip(*processed_bad_data)
            rewards = prm.embeddings.create(
                input=input_ids,
                model=args.prm_name_or_path.split("/")[-1],
            )
