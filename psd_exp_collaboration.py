...
def get_responses(args, client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, prompts, problems):
    ...
    while current_prompts:
        ...
        draft_step_rewards = derive_step_rewards_vllm(rewards, reward_flags) # list[list]
        ...
        # Evaluate responses from client1 with PRM
        full_responses = [''.join(r[0] for r in prev_resp) + new_resp.text
                    for (_, _, prev_resp), new_resp in zip(current_prompts, responses2)]
        processed_data = [
            prepare_input(p, full_resp, tokenizer=tokenizer_prm, step_token=args.step_word) 
            for p, full_resp in zip(current_problems, full_responses)
        ]
        ...
        target_step_rewards = derive_step_rewards_vllm(rewards, reward_flags) # list[list]
        ...
        for (orig_idx, prompt, prev_responses), response1, response2, draft_step_reward, target_step_reward in zip(current_prompts, responses1, responses2, draft_step_rewards, target_step_rewards):
            draft_rewards[orig_idx].append(round(draft_step_reward[-1], 3))
            target_rewards[orig_idx].append(round(target_step_reward[-1], 3))
            if draft_step_reward[-1] >= target_step_reward[-1]:
                good_prompts.append((orig_idx, prompt, prev_responses, response1, True))  # True means use client1
            else:
                good_prompts.append((orig_idx, prompt, prev_responses, response2, True))  # True means use client1
    ...
    return outputs, draft_rewards, target_rewards, draft_responses, target_responses
...
def main(client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        outputs, draft_rewards, target_rewards, draft_responses, target_responses = get_responses(
            args,
            client1, 
            client2,
            prm,
