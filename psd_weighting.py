...
def prepare_data(data_name, args):
    ...
    out_file = f"{output_dir}/{data_name}/{out_file_prefix}_s{args.start}_e{args.end}_weighting{args.weight_version}_prm{args.max_prm_threshold}_maxturns{args.max_turns}.jsonl"
...
def get_responses(args, client1, client2, prm, tokenizer1, tokenizer2, tokenizer_prm, prompts, problems):
    ...
    while current_prompts:
        prm_threshold = args.max_prm_threshold
        ...
        for (orig_idx, prompt, prev_responses), response1, step_reward in zip(current_prompts, responses1, step_rewards):
            ...
            if args.weight_version == "v1": # w(r) = 1(r >= delta)
            ...
            elif args.weight_version == "v0": # w(r) = p
                if prm_threshold >= random.random():
                    good_prompts.append((orig_idx, prompt, prev_responses, response1, True))  # True means use client1
                else:
                    response1_text = response1.text + args.step_word
                    token_counts[orig_idx] = (token_counts[orig_idx][0], token_counts[orig_idx][1], token_counts[orig_idx][2]+len(tokenizer1.encode(response1_text)))
                    bad_prompts.append((orig_idx, prompt, prev_responses))
            elif args.weight_version == "v2": # w(r) = min(1, max(0, r))
                if step_reward[-1] >= random.random():
                    good_prompts.append((orig_idx, prompt, prev_responses, response1, True))  # True means use client1
                else:
                    response1_text = response1.text + args.step_word
                    token_counts[orig_idx] = (token_counts[orig_idx][0], token_counts[orig_idx][1], token_counts[orig_idx][2]+len(tokenizer1.encode(response1_text)))
                    bad_prompts.append((orig_idx, prompt, prev_responses))
            elif args.weight_version == "v3": # w(r) = max(0, r/(1+r))
                if step_reward[-1] / (1+step_reward[-1]) >= random.random():
                    good_prompts.append((orig_idx, prompt, prev_responses, response1, True))  # True means use client1
                else:
                    response1_text = response1.text + args.step_word
                    token_counts[orig_idx] = (token_counts[orig_idx][0], token_counts[orig_idx][1], token_counts[orig_idx][2]+len(tokenizer1.encode(response1_text)))
