...
def setup(args):
    ...
    rm_tokenizer = get_tokenizer(args.rm_name_or_path)
...
def get_responses(args, draft_client, draft_tokenizer, target_client, target_tokenizer, rm, rm_tokenizer, prompts, problems):
    ...
    while current_prompts:
        ...
        step_rewards = prm_scores(rm, rm_tokenizer, current_prompts, current_problems, responses1)

        # Split prompts based on step_reward
        good_prompts = []
        bad_prompts = []
        bad_problems = []
        for problem, (orig_idx, prompt, prev_responses), response1, step_reward in zip(current_problems, current_prompts, responses1, step_rewards):
        ...
        if bad_prompts:
            ...
            step_rewards = prm_scores(rm, rm_tokenizer, bad_prompts, bad_problems, responses2)
...
def main(draft_client, draft_tokenizer, target_client, target_tokenizer, rm, rm_tokenizer, data_name, args):
    ...
    problems = [sample["instruction"] for sample in samples]
    outputs, token_counts, turn_info, draft_rewards, target_rewards, draft_responses, target_responses = get_responses(args, draft_client, draft_tokenizer, target_client, target_tokenizer, rm, rm_tokenizer, prompts, problems)
