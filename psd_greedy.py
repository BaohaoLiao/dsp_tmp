...
def parse_args():
    ...
    parser.add_argument("--llm1_ip_address", default="http://localhost:12341/v1", type=str)
    parser.add_argument("--llm2_name_or_path", default="Qwen2.5-Math-1.5B-Instruct", type=str)
    parser.add_argument("--llm2_ip_address", default="http://localhost:12342/v1", type=str)
    ...
    parser.add_argument("--num_greedy_steps", type=int, default=1)
    parser.add_argument(
        "--apply_chat_template",
        action="store_true",
...
def setup(args):
    ...
    tokenizer_prm = AutoTokenizer.from_pretrained(args.prm_name_or_path, trust_remote_code=True)

    # infer & eval
    data_list = args.data_names.split(",")
    results = []
    for data_name in data_list:
        results.append(main(client1, prm, tokenizer1, tokenizer_prm, data_name, args))
...
def group_sublist(input_list, number):
    grouped_prompts = []
    for i in range(0, len(input_list), number):
        group = input_list[i:i+number]
        grouped_prompts.extend([group])
    return grouped_prompts

def get_responses(args, client1, prm, tokenizer1, tokenizer_prm, prompts, problems):
    ...
    while current_prompts:
        ...
        responses1 = client1.completions.create(
            ...
            n=args.num_greedy_steps,
            stop=[args.step_word],
        ).choices
        responses1 = sorted(responses1, key=lambda x: int(x.index))

        # Evaluate responses from client1 with PRM
        if args.num_greedy_steps > 1:
            current_prompts = [prompt for prompt in current_prompts for _ in range(args.num_greedy_steps)]
            current_problems = [p for p in current_problems for _ in range(args.num_greedy_steps)]
        ...
        good_prompts = []
        grouped_prompts = group_sublist(current_prompts, args.num_greedy_steps)
        grouped_responses1 = group_sublist(responses1, args.num_greedy_steps)
        grouped_step_rewards = group_sublist(step_rewards, args.num_greedy_steps)
        for p_list, r_list, s_list in zip(grouped_prompts, grouped_responses1, grouped_step_rewards):
            s_last = [s[-1] for s in s_list]
            response1 = r_list[np.argmax(s_last)]
            orig_idx, prompt, prev_responses = p_list[np.argmax(s_last)]
            good_prompts.append((orig_idx, prompt, prev_responses, response1, True))
...
def main(client1, prm, tokenizer1, tokenizer_prm, data_name, args):
